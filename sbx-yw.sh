...
# 修复记录:
#   1-19. (前述所有修复与增强)
#  20. 全局本机身份备注 (落地机/中转机一目了然)
#  21. 【重磅新增】Reality 支持集群模式 (单端口挂多落地机，自动负载均衡)
# ============================================================================
    ...
    for ((i=0; i<count; i++)); do
        ...
        case "$type" in
            vless-reality)
                ...
                uuid=$(jq -r ".[$i].uuid" "$RULESJSON"); port=$(jq -r ".[$i].port" "$RULES_JSON")
                sni=$(jq -r ".[$i].sni" "$RULES_JSON"); fp=$(jq -r ".[$i].fp" "$RULES_JSON")
                pub=$(jq -r ".[$i].pub_key" "$RULESJSON"); sid=$(jq -r ".[$i].sid" "$RULES_JSON")
            ...
            vless-reality)
                if [ "$mode" == "standalone" ]; then
                    json=$(echo "$json" | jq --argjson p "$port" --arg in_tag "$in_tag" \
                        --arg u "$(jq -r ".[$i].uuid" "$RULES_JSON")" --arg pk "$(jq -r ".[$i].priv_key" "$RULESJSON")" \
                    ...
                    # 【集群模式】生成 1个入站 + N个出站 + 1个 urltest 负载均衡
                    local out_tags=""
                    local idx=0
